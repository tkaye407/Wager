    ...
    @IBAction func SignUpPressed(_ sender: Any) {
      FIRAuth.auth()!.createUser(withEmail: self.EmailText.text!, password: self.PasswordText.text!) {user, error in
        if error == nil {
          ...
          let profItem = Profile(firstName: self.FirstNameText.text!, lastName: self.LastNameText.text!, email: self.EmailText.text!, pnl: 0, age: Int((self.AgeText.text!))!, venmoID: self.VenmoIdText.text!, gender: self.GenderText.text!, key: (user?.uid)!, userID: self.UsernameText.text!)
